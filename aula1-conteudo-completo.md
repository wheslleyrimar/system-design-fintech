...
**Particionamento do ledger — o desenho concreto.** Para evitar que um único banco de dados fique sobrecarregado, podemos dividir as contas entre várias partições. Cada conta é enviada sempre para uma dessas partições usando uma regra simples. Por exemplo, `hash(conta_id) mod N` significa transformar o identificador da conta em um número e usar esse número para decidir em qual das **N partições** aquela conta ficará. Na prática, é só uma forma automática de espalhar as contas de maneira equilibrada.

Assim, transações que acontecem dentro da mesma partição continuam simples e rápidas. O desafio aparece quando a transação envolve contas de partições diferentes — por exemplo, Ana em uma partição e Bruno em outra. Nesse caso, o sistema precisa coordenar os dois lados.

Existem duas abordagens comuns: **Two-Phase Commit**, em que todas as partições precisam confirmar a operação juntas, garantindo atomicidade, mas com mais latência e risco de bloqueios; ou **Saga**, em que cada etapa é concluída separadamente e, se alguma etapa posterior falhar, uma operação de compensação corrige o que já foi feito. A Saga tende a escalar melhor, mas aceita que, por um curto período, as partições possam ficar em estados diferentes até que o processo termine.

Essa ideia é parecida com o padrão **Outbox**: em vez de assumir que tudo vai acontecer perfeitamente dentro de uma única transação, o sistema registra de forma durável o que ainda precisa acontecer e processa essas etapas de maneira controlada — o padrão que a Aula 2 vai apresentar, e que a Aula 8 leva até o ADR-003.
