...
1. O app da Ana chama o **Gateway/BFF** — o porteiro do sistema (BFF é *Backend for Frontend*: um backend feito sob medida para servir o app). É ele quem faz a **terminação TLS** (desencripta a conexão segura que veio da internet, para o resto do sistema trabalhar com o conteúdo), aplica o **rate limit** (o limite de quantas requisições cada cliente pode fazer por segundo, para ninguém — nem um app com bug — afogar o sistema) e confere a **idempotência** da Aula 1, usando o **E2E ID** — o identificador único de ponta a ponta que todo Pix carrega: se a mesma transação chegar duas vezes, ela é reconhecida e não executa de novo. Tudo isso **antes** de qualquer lógica de negócio.
...
E aqui está o conselho operacional que cai direto dessa tabela, e que eu quero que vocês levem para a vida: **nunca dimensionem um sistema financeiro para operar acima de 70% de utilização no pico.** Aquela folga que parece desperdício de dinheiro é literalmente o que separa "lento" de "fora do ar".

E de onde vem esse número — quem decretou os 70%? Sejamos honestos sobre a natureza dele, porque isso também é aula: **não existe um teorema dos 70%.** A curva é lisa; matematicamente, não há um ponto exato onde ela "quebra" — Neil Gunther, uma das referências em planejamento de capacidade, gosta de provocar dizendo que o "joelho" da curva é folclore. O que existe é a tabela que a gente acabou de montar: a 70%, a espera é ~2,3 atendimentos e ainda se opera; a 80%, ela dobra; a 90%, quadruplica de novo. Os 70% são uma **regra de bolso de engenharia de capacidade** — o ponto de corte que a indústria adotou porque é a última faixa em que um pico inesperado ainda encontra folga antes da parede. E vocês encontram essa régua operacionalizada por aí: no auto scaling do DynamoDB, a [documentação da AWS](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/AutoScaling.html) usa **70% como a utilização-alvo do exemplo canônico** — numa faixa configurável que vai de 20% a 90%; e o livro de SRE do Google, no capítulo [Addressing Cascading Failures](https://sre.google/sre-book/addressing-cascading-failures/), trata "operar perto da capacidade" como uma das causas clássicas de falha em cascata, e manda provisionar folga deliberada — a regra **N+2**: capacidade para perder duas réplicas e continuar de pé. Para um sistema financeiro, com tráfego em rajada e teto regulatório de latência, escolher o lado conservador dessa régua não é gordura — é projeto.

Quando alguém na sua empresa disser "esses servidores estão a 40%, dá para cortar metade", vocês agora têm a tabela — e as fontes — para responder.
...
Se vocês já viram o diagrama clássico de payment system do Alex Xu — publicado no artigo "[Designing a Payment System](https://newsletter.pragmaticengineer.com/p/designing-a-payment-system)", do Pragmatic Engineer, e no *System Design Interview vol. 2*: aquele com PSP, settlement file e uma caixa de *Reconciliation* recebendo o arquivo de fora da fronteira — reconheçam a estrutura: é exatamente esta. Lá, o texto diz: toda noite o PSP manda um arquivo de settlement com o saldo e as transações do dia, e o sistema de reconciliação compara esse arquivo com o ledger. Muda o nome do trilho (lá, adquirência e bandeiras; aqui, SPI e Conta PI), não muda o papel: **a reconciliação é o sistema que salva vocês quando todos os outros falharem em silêncio.** Como esse componente se opera no dia a dia — o que monitorar, quando alarmar, o que é taxa de divergência aceitável — é assunto da Aula 7.
