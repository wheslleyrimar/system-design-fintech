...
**"Adotar o Spec Kit não é se amarrar numa ferramenta que pode morrer daqui a um ano?"**

Pode morrer, sim — e a resposta honesta é que isso importa menos do que parece. Os artefatos que o Spec Kit gera são Markdown num repositório Git: `constitution.md`, `spec.md`, `plan.md`, `tasks.md`. Se a ferramenta sumir amanhã, os arquivos continuam legíveis por qualquer humano e qualquer agente — não existe formato proprietário, não existe servidor do qual migrar. O que vocês estariam perdendo são os comandos de conveniência, não o conteúdo. E o que eu peço que vocês internalizem não é a sintaxe `/speckit.*`, é a **ordem imposta**: princípios antes da spec, spec antes do plano, plano antes das tarefas, verificação cruzada antes do código. Essa ordem é o SDD; a ferramenta é só o trilho mais confortável disponível hoje. Comparem com o ADR: o formato do Nygard sobreviveu a dez gerações de ferramenta porque o valor está na disciplina, não no tooling.

**"Constituição e ADR não são a mesma coisa duas vezes? Vou manter os dois sincronizados na mão?"**

São artefatos diferentes com uma relação deliberada, e a analogia que eu uso é jurídica: o ADR é a **jurisprudência** — cada decisão registrada com contexto, alternativas rejeitadas e consequências, imutável, datada; a constituição é a **lei consolidada** — o destilado operacional do que ficou decidido, escrito como regra que um agente verifica a cada feature. O ADR-001 explica *por que* o ledger é fortemente consistente, com o custo assumido; a constituição só diz "Σ débitos = Σ créditos, sempre" — porque é isso que o `/speckit.analyze` precisa checar em milissegundos, sem reler a história toda. A sincronização não é "na mão": quando um novo ADR muda uma regra, a alteração na constituição entra no mesmo pull request — e uma linha da constituição sem ADR que a justifique é exatamente o tipo de coisa que a revisão deve barrar. Um aponta para o outro, como lei aponta para a exposição de motivos.
